...
def caculate_index(path,ncct_path, path_save):
    ...
    # out = sitk.GetImageFromArray(img_array)  #基线模型不需要执行
...
def aortic_index_xs(path,path_h5):
    ...
    lumen_mask[lumen_mask > 1] = 0
    lumen_mask = remove_small_volums(lumen_mask)
    diameter_per_lumenc,area_per_lumenc=diameter_arear(lumen_mask)#
    per_indexc = [diameter_per_lumenc[i] / diameter_per_total[i] if diameter_per_total[i] != 0 else 0 for i in range(len(diameter_per_total))]
    per_indexc= np.array(per_indexc)
    per_indexc[per_indexc==0]=1
    per_indexc=1-per_indexc
...
def Aortic_index_external_xcs():
    f = open("/media/bit301/data/yml/project/python39/p2/Aorta_net/data/list/discard.txt")  # hnnk test.txt
    dis_list = []
    for line in f.readlines():  # tile_step_size=0.75较好处理官腔错位问题
...
def models_index_external_xcs():
    f = open("/media/bit301/data/yml/project/python39/p2/process/list/discard.txt")  # hnnk test.txt
    dis_list = []
    for line in f.readlines():  # tile_step_size=0.75较好处理官腔错位问题
        path=line.split('\n')[0].replace("0.nii.gz","2.nii.gz").split("external/")[1]
        dis_list.append(path)

    # models=["SwinUNETR","unet","p2ux","p2nnf","MedNeXt","p2res",]
    models = ["Aortic_index_v1"]
    # models = ["unet","p2ux"]#p2ux 10
    for model in models:
        path = "/media/bit301/data/yml/data/p2_nii/test/"+model
        path_list = []
        for root, dirs, files in os.walk(path, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                if "/2.nii.gz" in path and "cq/" in path: #
                    pp = path.split(model+"/")[1]
                    if pp in dis_list:
                        continue
                    path_list.append(path)
        i = 0
        for path in path_list:
            ...
            # path = "/media/bit301/data/yml/data/p2_nii/external/cq/dis/xz/PA247/2.nii.gz"
            path_ncct = path.replace("2.nii.gz", "0.nii.gz")
            path_ncct=path_ncct.replace("test/"+model,"external")
            caculate_index(path, path_ncct, path)
            i = i + 1
            if i % 10 == 0:
                print('numbers:', i)
...
def conf_index(confusion_matrix):
    ...
    sub_ACC = np.where(TP + FN > 0, TP / (TP + FN + epsilon), 0)  # accuracy of each class

    # 总体准确率
    acc = (TP.sum() + TN.sum()) / confusion_matrix.sum()
    # 平均准确率（Macro-average）
    ...
    # Weighted F1
    Weighted_F1 = (F1_Score * weight).sum()
...
def stuart_maxwell_test(table):
    """
    执行Stuart-Maxwell检验以评估配对分类变量的一致性。

    参数:
    table : array-like, 二维方阵，表示配对数据的列联表。

    返回:
    chi2_stat : float，卡方统计量
    p_value : float，检验的p值
    """
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                S[i, j] = row_sums[i] + col_sums[i] - 2 * table[i, i]
            else:
                S[i, j] = - (table[i, j] + table[j, i])
    ...
    try:
        ...
        S_inv = np.linalg.pinv(S_reduced)

    # 计算卡方统计量
    chi2_stat = d_reduced.T @ S_inv @ d_reduced
    df = n - 1  # 自由度
...
def caculate_statis():
    ...
    for i in range(len(dataset_files)):
        ...
        for ij in range(len(locations)):
            ...
            for typee in range(len(types)):
                ...
                # mm = "matrix_"+ str(locations[ij]) + types[typee]#total，三个数据集汇总
                save_name=mm
                ...
                for path in path_list:
                    ...
                    with h5py.File(path, 'r') as f_gd:#评估四分位数据
                        diameter_per_lumen = f_gd['diameter_per_lumen'][:]*0.67 #像素系数 0.67
                        # area_per_lumen = f_gd['area_per_lumen'][:]
                        diameter_per_total = f_gd['diameter_per_total'][:]*0.67
                        # area_per_total = f_gd['area_per_total'][:]
                        if typee==0:
                            per_index = f_gd['per_index'][:]
                        elif typee==1:
                            ...
                            per_index = f_gd['per_indexs'][:]
                        ncct_arrayyy = f_gd['ncct_arrayyy'][:]
                        ncct_arrayyy = np.where(ncct_arrayyy <5, 0,ncct_arrayyy)#剔除微小钙化

                    path_mea=path.replace("2.h5","22.h5")
                    with h5py.File(path_mea, 'r') as f_mea:#评估四分位数据
                        diameter_per_lumen_mea = f_mea['diameter_per_lumen'][:]*0.67
                        area_per_lumen_mea = f_mea['area_per_lumen'][:]
                        diameter_per_total_mea = f_mea['diameter_per_total'][:]*0.67
                        ...
                        ncct_arrayyy_mea = np.where(ncct_arrayyy_mea < 5, 0, ncct_arrayyy_mea)
                    ...
                    if np.all(per_index == 1) or np.all(per_index_mea == 1):
                        ...
                        print(path+"_mea")
                    write_path = path.split('Aortic_indexcs')[1]
                    ...
                    # ca_gd=ncct_arrayyy[start_index:end_index]*0.67*0.67
                    # ca_mea =ncct_arrayyy_mea[start_index:end_index]*0.67*0.67
                    ca_gd=np.sum(ncct_arrayyy[start_index:end_index])*0.18704167#体积系数 1.25x0.67x0.67/3
                    ca_mea = np.sum(ncct_arrayyy_mea[start_index:end_index])*0.18704167
                    # ca_gd = np.sum(ca_gd) * 1.25/3  # 体积系数
                    # ca_mea = np.sum(ca_mea) *1.25/3
                    if ca_mea <= 0:
                        cal_mea=1
                    elif 0 <ca_mea <= 99:
                        cal_mea=2
                    ...
                    if location==1:
                        su_lumenn_1.append(lumnen)
                        su_totall_1.append(total)
                        if typee == 0:
                            su_stenosis_t1.append(stenosis_index)
                            su_flag_gd_t1.append(stenosis_gd)
                            su_flag_mea_t1.append(stenosis_mea)
                            su_caa_gd_t1.append(cal_gd)
                            su_caa_mea_t1.append(cal_mea)
                        elif typee==1:
                            su_stenosis_c1.append(stenosis_index)
                        elif typee == 2:
                            su_stenosis_s1.append(stenosis_index)
                    elif location==2:
                        su_lumenn_2.append(lumnen)
                        su_totall_2.append(total)
                        if typee == 0:
                            su_stenosis_t2.append(stenosis_index)
                            su_flag_gd_t2.append(stenosis_gd)
                            su_flag_mea_t2.append(stenosis_mea)
                            su_caa_gd_t2.append(cal_gd)
                            su_caa_mea_t2.append(cal_mea)
                        elif typee==1:
                            su_stenosis_c2.append(stenosis_index)
                        elif typee == 2:
                            su_stenosis_s2.append(stenosis_index)

                    # ii = ii + 1
                    # if ii % 10 == 0:
                    #     print('numbers:', ii)
                print(save_name)
                xlsx="./xlsx/"+save_name+'.xlsx'
                out_put = os.path.dirname(xlsx)
                if not os.path.isdir(out_put):
                    os.makedirs(out_put)
                data.save(xlsx) #
                mean_lumnen = np.nanmean(lumenn, axis=0)  # 列平均
                print("mean_lumnen:", mean_lumnen)
                std_lumnen = np.nanstd(lumenn, axis=0)  # 列平均
                print("std_lumnen:", std_lumnen)

                mean_total = np.nanmean(totall, axis=0)  # 列平均
                print("mean_vaseel:", mean_total)
                std_vaseel = np.nanstd(totall, axis=0)  # 列平均
                print("std_vaseel:", std_vaseel)

                mean_stenosis = np.nanmean(stenosis, axis=0)  # 列平均
                print("mean_stenosis:", mean_stenosis)
                std_stenosis = np.nanstd(stenosis, axis=0)  # 列平均
                print("std_stenosis:", std_stenosis)
                cm = confusion_matrix(flag_gd, flag_mea)
                if ij==0 and typee==0:
                    su_confusion_t1.append(cm)
                elif ij==0 and typee==1:
                    su_confusion_c1.append(cm)
                elif ij == 0 and typee == 2:
                    su_confusion_s1.append(cm)
                elif ij==1 and typee==0:
                    su_confusion_t2.append(cm)
                elif ij==1 and typee==1:
                    su_confusion_c2.append(cm)
                elif ij == 1 and typee == 2:
                    su_confusion_s2.append(cm)
                print(f"\nconfusion_matrix:\n{cm}")
                conf_index(cm)#依据混淆矩阵计算
                # kappa = cohen_kappa_score(np.array(flag_gd), np.array(flag_mea))
                # print("kappa:", kappa)# Kappa 统计量
                # stat, p1 = ss.ranksums(np.array(flag_gd), np.array(flag_mea))# 进行 Wilcoxon 秩和检验
                chi2_stat, p_value = stuart_maxwell_test(cm)  # 执行 Stuart-Maxwell Test
                print(f"卡方统计量: {chi2_stat}, p值: {p_value}")

                mcc = matthews_corrcoef(np.array(flag_gd), np.array(caa_mea))# Matthews 相关系数
                print("mcc:", mcc)
                ...
                if ij==0 and typee==1:
                    su_confusion_ca1.append(cm)
                elif ij==1 and typee==1:
                    su_confusion_ca2.append(cm)
                print(f"\nconfusion_matrix_ca:\n{cm}")
                conf_index(cm)  # 依据混淆矩阵计算
                chi2_stat, p_value = stuart_maxwell_test(cm)  # 执行 Stuart-Maxwell Test
                print(f"卡方统计量: {chi2_stat}, p值: {p_value}")
                # kappa = cohen_kappa_score(np.array(caa_gd), np.array(caa_mea))
                # print("kappa:", kappa)  # Kappa 统计量
                mcc = matthews_corrcoef(np.array(caa_gd), np.array(caa_mea))  # Matthews 相关系数
                print("mcc:", mcc)
    ...
    print("total dataset-location 1")
    mean_lumnen = np.nanmean(su_lumenn_1, axis=0)  # 列平均
    print("mean_lumnen:", mean_lumnen)
    std_lumnen = np.nanstd(su_lumenn_1, axis=0)  # 列平均
    print("std_lumnen:", std_lumnen)
    mean_total = np.nanmean(su_totall_1, axis=0)  # 列平均
    print("mean_vaseel:", mean_total)
    std_vaseel = np.nanstd(su_totall_1, axis=0)  # 列平均
    print("std_vaseel:", std_vaseel)
    mean_stenosis = np.nanmean(su_stenosis_t1, axis=0)  # 列平均
    print("mean_stenosis_t1:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_t1, axis=0)  # 列平均
    print("std_stenosis_t1:", std_stenosis)
    mean_stenosis = np.nanmean(su_stenosis_c1, axis=0)  # 列平均
    print("mean_stenosis_c1:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_c1, axis=0)  # 列平均
    print("std_stenosis_c1:", std_stenosis)
    mean_stenosis = np.nanmean(su_stenosis_s1, axis=0)  # 列平均
    print("mean_stenosis_s1:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_s1, axis=0)  # 列平均
    print("std_stenosis_s1:", std_stenosis)

    print("total dataset-location 2")
    mean_lumnen = np.nanmean(su_lumenn_2, axis=0)  # 列平均
    print("mean_lumnen:", mean_lumnen)
    std_lumnen = np.nanstd(su_lumenn_2, axis=0)  # 列平均
    print("std_lumnen:", std_lumnen)
    mean_total = np.nanmean(su_totall_2, axis=0)  # 列平均
    print("mean_vaseel:", mean_total)
    std_vaseel = np.nanstd(su_totall_2, axis=0)  # 列平均
    print("std_vaseel:", std_vaseel)
    mean_stenosis = np.nanmean(su_stenosis_t2, axis=0)  # 列平均
    print("mean_stenosis_t2:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_t2, axis=0)  # 列平均
    print("std_stenosis_t2:", std_stenosis)
    mean_stenosis = np.nanmean(su_stenosis_c2, axis=0)  # 列平均
    print("mean_stenosis_c2:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_c2, axis=0)  # 列平均
    print("std_stenosis_c2:", std_stenosis)
    mean_stenosis = np.nanmean(su_stenosis_s2, axis=0)  # 列平均
    print("mean_stenosis_s2:", mean_stenosis)
    std_stenosis = np.nanstd(su_stenosis_s2, axis=0)  # 列平均
    print("std_stenosis_s2:", std_stenosis)

    out_put = "confuse_su_disp"
    if not os.path.isdir(out_put):
        os.makedirs(out_put)
    su_confusion = np.sum(su_confusion_t1, axis=0)
    print(f"su_confusion_t1:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    chi2_stat, p_value = stuart_maxwell_test(su_confusion)  # 执行 Stuart-Maxwell Test
    print(f"卡方统计量: {chi2_stat}, p值: {p_value}")
    save_path = os.path.join(out_put, "su_confusion_t1")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_t2, axis=0)
    print(f"su_confusion_t2:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    chi2_stat, p_value = stuart_maxwell_test(su_confusion)  # 执行 Stuart-Maxwell Test
    print(f"卡方统计量: {chi2_stat}, p值: {p_value}")
    save_path = os.path.join(out_put, "su_confusion_t2")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_c1, axis=0)
    print(f"su_confusion_c1:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    save_path = os.path.join(out_put, "su_confusion_c1")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_c2, axis=0)
    print(f"su_confusion_c2:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    save_path = os.path.join(out_put, "su_confusion_c2")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_s1, axis=0)
    print(f"su_confusion_s1:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    save_path = os.path.join(out_put, "su_confusion_s1")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_s2, axis=0)
    print(f"su_confusion_s2:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    save_path = os.path.join(out_put, "su_confusion_s2")#    "matrix_a" "matrix_b"
    confuse_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_ca1, axis=0)
    print(f"su_confusion_ca1:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    chi2_stat, p_value = stuart_maxwell_test(su_confusion)  # 执行 Stuart-Maxwell Test
    print(f"卡方统计量: {chi2_stat}, p值: {p_value}")
    save_path = os.path.join(out_put, "su_confusion_ca1")#    "matrix_a" "matrix_b"
    confuse_ca_plot(su_confusion,save_path)

    su_confusion = np.sum(su_confusion_ca2, axis=0)
    print(f"su_confusion_ca2:\n\n{su_confusion}")
    conf_index(su_confusion)  # 依据混淆矩阵计算
    chi2_stat, p_value = stuart_maxwell_test(su_confusion)  # 执行 Stuart-Maxwell Test
    print(f"卡方统计量: {chi2_stat}, p值: {p_value}")
    save_path = os.path.join(out_put, "su_confusion_ca2")#    "matrix_a" "matrix_b"
    confuse_ca_plot(su_confusion,save_path)
    print('finished!')

def baseline_models_statis():
    f = open("/media/bit301/data/yml/project/python39/p2/process/list/discard.txt")  # hnnk test.txt
    dis_list = []
    for line in f.readlines():  # tile_step_size=0.75较好处理官腔错位问题
        path=line.split('\n')[0].replace("0.nii.gz","2.h5").split("external/")[1]
        ...
        for ij in range(len(locations)):
            ...
            for typee in range(len(types)):
                ...
                for path in path_list:
                    ...
                    # path="/media/bit301/data/yml/data/p2_nii/Aortic_index/cq/nor/PA280/2.h5"
                    with h5py.File(path, 'r') as f_gd:#评估四分位数据
                        diameter_per_lumen = f_gd['diameter_per_lumen'][:]*0.67
                        # area_per_lumen = f_gd['area_per_lumen'][:]
                        diameter_per_total = f_gd['diameter_per_total'][:]*0.67
                        # area_per_total = f_gd['area_per_total'][:]
                        if typee==0:
                    ...
                    path_mea=path.replace("statisticians/Aortic_indexcs","test/p2nnf")#SwinUNETR。。
                    with h5py.File(path_mea, 'r') as f_mea:#评估四分位数据
                        diameter_per_lumen_mea = f_mea['diameter_per_lumen'][:]*0.67
                        area_per_lumen_mea = f_mea['area_per_lumen'][:]
                        diameter_per_total_mea = f_mea['diameter_per_total'][:]*0.67
                        ...
                        if typee == 0:
                            ...
                            per_index_mea = f_mea['per_indexs'][:]
                        ncct_arrayyy_mea = f_mea['ncct_arrayyy'][:]
                        ncct_arrayyy_mea = np.where(ncct_arrayyy_mea <5, 0,ncct_arrayyy_mea)#剔除微小钙化
                    ...
                    ca_gd=np.sum(ncct_arrayyy[start_index:end_index])*0.1870417#体积系数 1.25x0.67x0.67/3
                    ca_mea = np.sum(ncct_arrayyy_mea[start_index:end_index])*0.1870417
...
def confuse_total_ca():
 ...
 [  1,   0,   0, 156]]
    ...
    if not os.path.isdir(out_put):
        os.makedirs(out_put)
    save_path = os.path.join(out_put, "matrix_b")#    "matrix_a" "matrix_b"
    confuse_ca_plot(cm,save_path)
    print('finished!')
...
def remove_data():
    model="p2res" #p2res
    path = "/media/bit301/data/yml/data/p2_nii/test/" + model+"aa"
    path_list = []
    for root, dirs, files in os.walk(path, topdown=False):
        for file in files:
            path = os.path.join(root, file)
    ...
    models=["SwinUNETR","unet","p2ux","p2nnf","MedNeXt"]#496 495 493  493  496
    for model in models:
        path = "/media/bit301/data/yml/data/p2_nii/test/"+model
        i=0
        for root, dirs, files in os.walk(path, topdown=False):
            for file in files:
                ...
                if "/2.nii.gz" in path:
                    ...
                    if pp in path_list:
                        ...
                        print(path)
                        aa=path.split("/2.nii.gz")[0]
                        shutil.rmtree(aa)
                    i = i + 1
                    if i % 10 == 0:
                        print('numbers:', i)

def diagnosis_prepare():
    f = open("/media/bit301/data/yml/project/python39/p2/Aorta_net/data/discard.txt")  # hnnk test.txt
    dis_list = []
    for line in f.readlines():  # tile_step_size=0.75较好处理官腔错位问题
        path=line.split('\n')[0].replace("0.nii.gz","2.nii.gz").split("external/")[1]
    ...
    for root, dirs, files in os.walk(path, topdown=False):
        for file in files:
            path = os.path.join(root, file)
            if "/0.nii.gz" in path:
                pp = path.split("external/")[1]
                if pp in dis_list:
                    continue
                path_list.append(path)

    data = openpyxl.load_workbook('demo2.xlsx')
    table = data.active
    i=0
    for path in path_list:
        outpath=path.replace("external","exterald")
        out_put = os.path.dirname(outpath)
        if not os.path.isdir(out_put):
            os.makedirs(out_put)
        shutil.copyfile(path,outpath)
        write_path = path.split('external')[1]
        table.cell(i + 1, 0).value = write_path
        i = i + 1
        if i % 10 == 0:
            print('numbers:', i)
    data.save('diagnosis.xlsx')  #
...
if __name__ == '__main__':
    ...
    # models_index_external_xcs()

    #counting the lesion number of train or test dataset
    # statis_train_lesion()
    # statis_test_lesion()

    # statisticians of evaluation（Aortic-AEE，baseline models）
    # caculate_statis()#已经将汇总混淆矩阵绘制整合进来,需要切换其中部分语句
    baseline_models_statis() #python ./aortic_index_1dataset.py  > record.txt

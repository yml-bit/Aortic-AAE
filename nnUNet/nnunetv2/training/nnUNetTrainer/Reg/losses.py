...
class NCC_new:
    ...
    def loss(self, y_true, y_pred, weight=None, return_per_loss=False, ignore_label=None):
        ...
        cross = IJ_sum - u_J * I_sum - u_I * J_sum + u_I * u_J * win_size
        I_var = I2_sum - 2 * u_I * I_sum + u_I * u_I * win_size  # 增加分母偏移量
        J_var = J2_sum - 2 * u_J * J_sum + u_J * u_J * win_size

        cc = cross * cross / (I_var * J_var+ 1e-5)
        # cc = torch.nan_to_num(cc, nan=0.01)  # 将nan值替换为0
        # q1 = torch.quantile(cc.float(), 0.05)
        # q2 = torch.quantile(cc.float(), 0.95)
        # print(q1)
        # print(q2)
        # print("\n")
        # cc=torch.clamp(cc,min=-10,max=10)
        if weight is not None:
            B = len(cc)
            assert len(weight) == B, "The length of data weights must be equal to the batch value."
            assert 0.99 < weight.sum().item() < 1.1, "The weights of data must sum to 1."
...
class Grad3d(torch.nn.Module):
    ...
    def forward(self, y_pred):
        ...
        if self.penalty == 'l2':
            ...
            # dz = dz * dz
            # 添加饱和操作，避免数值下溢
            dy_clipped = torch.clamp(dy, min=-1., max=1.) ** 2
            dx_clipped = torch.clamp(dx, min=-1., max=1.) ** 2
            dz_clipped = torch.clamp(dz, min=-1., max=1.) ** 2

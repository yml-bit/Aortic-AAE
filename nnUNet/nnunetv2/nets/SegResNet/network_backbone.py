...
class SegResNetVAE(SegResNet):
    ...
    def __init__(
        ...
        self.vae_conv_final = UnetOutBlock(spatial_dims=spatial_dims, in_channels=init_filters, out_channels=1)
    ...
    def _get_syn(self, net_input: torch.Tensor, vae_input: torch.Tensor):
        """
        Args:
    ...
    def forward(self, x):
        ...
        syn_input = x
        x= self.decode(x, down_x)
        syn=self._get_syn(net_input, syn_input)#x, syn
        return x, syn

...
class SpatialAttention3D(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.sigmoid(spatial_weights)

class CBAM3D(nn.Module):
    def __init__(self, in_channels, ratio=8, kernel_size=7):
        super(CBAM3D, self).__init__()
        self.channel_att = ChannelAttention3D(in_channels, ratio)
...
class DirectionalAttention3D(nn.Module):
    def __init__(self, in_channels, kernel_size=(3, 1, 1)):
        super().__init__()
        # 轴向(Z方向)注意力
        self.conv_z = nn.Conv3d(in_channels, 1, kernel_size, padding=(kernel_size[0] // 2, 0, 0))
        # 冠状面(Y方向)
        self.conv_y = nn.Conv3d(in_channels, 1, (1, 3, 1), padding=(0, kernel_size[0] // 2, 0))
        # 横断面(X方向)
        self.conv_x = nn.Conv3d(in_channels, 1, (1, 1, 3), padding=(0, 0, kernel_size[0] // 2))

    def forward(self, x):
        att_z = torch.sigmoid(self.conv_z(x))
        att_y = torch.sigmoid(self.conv_y(x))
        att_x = torch.sigmoid(self.conv_x(x))
        return x * (att_z + att_y + att_x) / 3

class DualAttention3D(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.channel_att = ChannelAttention3D(in_channels)  # 类似SE
        self.spatial_att = SpatialAttention3D()  # 类似CBAM
...
#调试版本：只在最后输出模块处添加级联注意力（没少效果，所以才有后面各个层级都添加级联注意力）
class Aorta1ddd(nn.Module):
...
class Aorta1(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            x_up_3 = checkpoint.checkpoint(self.up_3, x, self.dummy_tensor)
            x_res_3 = checkpoint.checkpoint(self.Cascade_attention3, x_res_3, self.dummy_tensor)
            dec_x = x_res_3 + x_up_3
            x = self.iterative_checkpoint(self.dec_block_3, dec_x)
            if self.do_ds:
                x_ds_3 = checkpoint.checkpoint(self.out_3, x, self.dummy_tensor)
            ...
            x_up_2 = checkpoint.checkpoint(self.up_2, x, self.dummy_tensor)
            x_res_2= checkpoint.checkpoint(self.Cascade_attention2, x_res_2, self.dummy_tensor)
            dec_x = x_res_2 + x_up_2
            x = self.iterative_checkpoint(self.dec_block_2, dec_x)
            if self.do_ds:
                x_ds_2 = checkpoint.checkpoint(self.out_2, x, self.dummy_tensor)
            ...
            x_up_1 = checkpoint.checkpoint(self.up_1, x, self.dummy_tensor)
            x_res_1 = checkpoint.checkpoint(self.Cascade_attention1, x_res_1, self.dummy_tensor)
            dec_x = x_res_1 + x_up_1
            x = self.iterative_checkpoint(self.dec_block_1, dec_x)
            if self.do_ds:
                x_ds_1 = checkpoint.checkpoint(self.out_1, x, self.dummy_tensor)
            ...
            x_up_0 = checkpoint.checkpoint(self.up_0, x, self.dummy_tensor)
            x_res_0= checkpoint.checkpoint(self.Cascade_attention, x_res_0, self.dummy_tensor)
            dec_x = x_res_0 + x_up_0
            x = self.iterative_checkpoint(self.dec_block_0, dec_x)
            del x_res_0, x_up_0, dec_x
...
class Aorta2a(nn.Module):
    ...
    def __init__(self,
        ...
        self.Cascade_attention = SE3D(n_channels)
        ...
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
            self.Cascade_attention1 = SE3D(n_channels* 2)
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim=dim)
            self.Cascade_attention2 = SE3D(n_channels* 4)
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim=dim)
            self.Cascade_attention3 = SE3D(n_channels* 8)
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim=dim)
            self.Cascade_attention4 = SE3D(n_channels* 16)
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim=dim)
        self.block_counts = block_counts
...
class Aorta2b(nn.Module):
    ...
    def __init__(self,
        ...
        self.Cascade_attention = CBAM3D(n_channels)
        ...
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
            self.Cascade_attention1 = CBAM3D(n_channels* 2)
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim=dim)
            self.Cascade_attention2 = CBAM3D(n_channels* 4)
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim=dim)
            self.Cascade_attention3 = CBAM3D(n_channels* 8)
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim=dim)
            self.Cascade_attention4 = CBAM3D(n_channels* 16)
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim=dim)
        self.block_counts = block_counts
...
#DirectionalAttention3D
class Aorta2c(nn.Module):
    ...
    def __init__(self,
        ...
        self.Cascade_attention = DirectionalAttention3D(n_channels)
        ...
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
            self.Cascade_attention1 = DirectionalAttention3D(n_channels* 2)
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim=dim)
            self.Cascade_attention2 = DirectionalAttention3D(n_channels* 4)
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim=dim)
            self.Cascade_attention3 = DirectionalAttention3D(n_channels* 8)
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim=dim)
            self.Cascade_attention4 = DirectionalAttention3D(n_channels* 16)
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim=dim)
        self.block_counts = block_counts
...
class Aorta2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.Cascade_attention = DualAttention3D(n_channels)
        ...
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
            self.Cascade_attention1 = DualAttention3D(n_channels* 2)
            self.out_1 = OutBlock(in_channels=n_channels * 2, n_classes=n_classes, dim=dim)
            self.Cascade_attention2 = DualAttention3D(n_channels* 4)
            self.out_2 = OutBlock(in_channels=n_channels * 4, n_classes=n_classes, dim=dim)
            self.Cascade_attention3 = DualAttention3D(n_channels* 8)
            self.out_3 = OutBlock(in_channels=n_channels * 8, n_classes=n_classes, dim=dim)
            self.Cascade_attention4 = DualAttention3D(n_channels* 16)
            self.out_4 = OutBlock(in_channels=n_channels * 16, n_classes=n_classes, dim=dim)
        self.block_counts = block_counts
...
class Aorta2cc(nn.Module):
    ...
    def __init__(self,
        ...
        self.Cascade_attention1 = Cascade_attention(n_channels, 1)

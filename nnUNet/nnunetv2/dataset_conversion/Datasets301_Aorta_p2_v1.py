...
def copy_files(src_data_folder: Path, train_dir: Path, labelTr_dir: Path, test_dir: Path,labelTs_dir: Path):
    ...
    for path_str in patients_train:
        ...
        if file.name == "0.nii.gz":
            ...
            se1output = labelTr_dir / f"Aorta_{str(num_cases).zfill(4)}.nii.gz"
            # shutil.copy(label_path, se1output)
            read = sitk.ReadImage(label_path, sitk.sitkInt16)  # 使用sitk重新保存，这样占用内存小很多
            label_array = sitk.GetArrayFromImage(read)
            label_array[label_array > 2.5] = 3 #预处理

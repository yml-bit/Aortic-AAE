...
import sys
# base = '/media/bit301/data/yml/project/python39/p2/DATASET'#512x512
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            if i == 0:
                trainer_name =self.modeln# checkpoint['trainer_name']
        ...
        trainer_class = recursive_find_python_class(join(nnunetv2.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'nnunetv2.training.nnUNetTrainer')#nnUNetTrainerMaCNN nnUNetTrainer
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(prediction,list):#新增
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                ...
                catch=self.network(torch.flip(x, (*axes,)))
                if isinstance(catch, list):#新增加
                    catch = catch[0]
                prediction += torch.flip(catch, (*axes,))
...
class Inference():
    def __init__(self,id:int,
        ...
        self.predictor = nnUNetPredictor(
            model=model,
            tile_step_size=0.5,  # p3_1a:0.5, p3_1b:0.75
            use_gaussian=True,
            use_mirroring=True,
            perform_everything_on_device=True,
            device=torch.device('cuda', int(id)),
            verbose=False,
            verbose_preprocessing=False,
            allow_tqdm=True
        )
        self.model=model
        model_path = 'Dataset301_Aorta/' + self.model + '__nnUNetPlans__3d_fullres'
        # model_path = 'Dataset301_Aorta/' + self.model + '__nnUNetPlans__3d_lowres'
    ...
    def predict_1a(self, input_dir, output_dir):
        ...
        path_list = []
        inputs = "/media/bit301/data/yml/data/p2_nii/external/"  # pre
        for root, dirs, files in os.walk(inputs, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                # if "0.nii.gz" in path:#and "cq" not in path
                if "0.nii.gz" in path and "hnnk" not in path:#not
                    pp = path.split("external/")[1]  # test
                    if pp in dis_list:
                        continue
        ...
        for path in path_list:
            ...
            ret = self.predictor.predict_single_npy_array(img, props, None, None, False)
            ret=remove_regions(ret)#直接计算最终的结果

            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out_put = path.replace("external", "test/"+self.model)
            out_put=out_put.replace("0.nii.gz","2.nii.gz")
            target_directory = os.path.dirname(out_put)
            os.makedirs(target_directory, exist_ok=True)
            sitk.WriteImage(label, out_put)
        ...
        # print("mean time:", time_sum/len(path_list))
        # print('Finished!\n')

if __name__ == '__main__':
    #sh用法：python predict_from_raw_data12.py nnUNetTrainer
    if len(sys.argv) < 3:
        print("Usage: python script.py <model> <input_dir>")
        sys.exit(1)
    id = int(sys.argv[1])
    model=sys.argv[2]

    #如果不是命令行运行，则将上述部分注释调，直接输入 model类型即可（注意，model是字符串）
    # id=0
    # model='nnUNetTrainerSNet2'# nnUNetTrainerMednext nnUNetTrainerMaCNN2
    inference = Inference(id,model)
    input_dir = "a"
    output_dir="b"
    inference.predict_1a(input_dir, output_dir)#